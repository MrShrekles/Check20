...
function renderWBTable() {
    ...
    const SECTIONS = [
        { group:'names',          label:'Names',             long:false, sub:['start','core','end'] },
    ...
    const html = SECTIONS.map(({ group, label, long, sub, scroll }) => {
        if (sub) {
            // Names: show sub-groups side by side
            const subBlocks = sub.map(sg => {
                ...
                return `
                    <div class="te-cat">
                        <div class="te-cat-label">${sg} (${entries.length})</div>
                        <div class="te-chips">
                            ${chips}
                            <button class="te-add-chip" onclick="addWBEntry('${group}','${sg}')">+ ${sg}</button>
                        </div>
                    </div>`;
            }).join('');
